...
const created = (data?: unknown): APIGatewayProxyResult => ({
  statusCode: 201,
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({ success: true, ...(data !== undefined && { data }) }),
});
...
const notFound = (message: string): APIGatewayProxyResult => error(message, 404);
 ...
 */
async function getTenant(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
 ...
 */
async function updateTenant(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
  const tenantId = event.pathParameters?.tenantId;
  if (!tenantId) {
    return error('tenantId is required');
  }
 ...
 */
async function deleteTenant(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
  const tenantId = event.pathParameters?.tenantId;
  if (!tenantId) {
    return error('tenantId is required');
  }
